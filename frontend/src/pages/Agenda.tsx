...
export function Agenda() {
  ...
  const load = async () => {
    ...
    try {
      const [a, p] = await Promise.all([api.get("/appointments"), api.get("/patients")]);
      setItems(a.data); setPatients(p.data.filter((x: Patient) => x.status === "ACTIVE"));
    } catch (err) { setError(getErrorMessage(err)); setItems([]); }
  ...
  async function remove(id: string) { if (!confirm("Excluir este agendamento?")) return; setError(""); try { await api.delete(`/appointments/${id}`); await load(); } catch (err) { setError(getErrorMessage(err)); } }
...
function LinkPatient({ id, name }: { id: string; name: string }) { return <Link to={`/pacientes/${id}`} className="font-semibold text-stone-700 hover:text-rosewood-600">{name}</Link>; }
