...
export function Agenda() {
  ...
  return (
    <>
      ...
      {loading ? <Loading /> : groups.length ? <div className="space-y-6">{groups.map(([date, appointments]) => <section key={date}><div className="mb-3 flex items-center gap-2"><CalendarPlus className="h-4 w-4 text-rosewood-500" /><h2 className="font-semibold capitalize text-stone-700">{new Date(`${date}T12:00:00Z`).toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long", timeZone: "UTC" })}</h2></div><div className="space-y-3">{appointments.map((item) => <div key={item.id} className="card flex flex-col gap-4 sm:flex-row sm:items-center"><div className="flex min-w-32 items-center gap-2 font-semibold text-stone-700"><Clock3 className="h-4 w-4 text-rosewood-400" />{item.startTime} — {item.endTime}</div><div className="flex-1"><LinkPatient id={item.patient.id} name={item.patient.name} /><p className="mt-1 text-sm text-stone-400">{careLabels[item.careType]}{item.notes ? ` · ${item.notes}` : ""}</p></div><span className={`w-fit rounded-full px-3 py-1 text-xs font-medium ${item.status === "SCHEDULED" ? "bg-rosewood-100 text-rosewood-700" : item.status === "COMPLETED" ? "bg-emerald-50 text-emerald-700" : "bg-stone-100 text-stone-600"}`}>{appointmentLabels[item.status]}</span><div className="flex gap-1"><button className="rounded-lg p-2 text-stone-400 hover:bg-white hover:text-rosewood-600" onClick={() => open(item)}><Pencil className="h-4 w-4" /></button><button className="rounded-lg p-2 text-stone-400 hover:bg-red-50 hover:text-red-500" onClick={() => remove(item.id)}><Trash2 className="h-4 w-4" /></button></div></div>)}</div></section>)}</div> : <EmptyState title="Nenhum atendimento agendado" description="Crie um agendamento para organizar sua rotina." action={<button className="btn-primary" onClick={() => open()}><Plus className="h-4 w-4" />Novo atendimento</button>} />}
