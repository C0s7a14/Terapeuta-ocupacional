...
type Tab = "overview" | "record" | "evolutions" | "documents";
...
export function PatientDetails() {
  ...
  const tabs: { key: Tab; label: string }[] = [{ key: "overview", label: "Visão geral" }, { key: "record", label: "Prontuário" }, { key: "evolutions", label: "Evoluções" }, { key: "documents", label: "Documentos" }];
  return (
    <>
      <div className="mb-5"><Link to="/pacientes" className="inline-flex items-center gap-2 text-sm text-stone-500 hover:text-rosewood-600"><ArrowLeft className="h-4 w-4" />Voltar para pacientes</Link></div>
      <div className="card mb-6">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
          <div className="flex items-center gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-rosewood-100 text-2xl font-semibold text-rosewood-600">{patient.name.split(" ").slice(0, 2).map((n) => n[0]).join("")}</div><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-semibold text-stone-800">{patient.name}</h1><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${patient.status === "ACTIVE" ? "bg-emerald-50 text-emerald-700" : "bg-stone-100 text-stone-500"}`}>{patient.status === "ACTIVE" ? "Ativo" : "Inativo"}</span></div><p className="mt-1 text-sm text-stone-500">{patient.mainCondition || "Condição principal não informada"}</p></div></div>
      ...
      <div className="mb-6 overflow-x-auto border-b border-stone-200"><div className="flex min-w-max gap-7">{tabs.map((item) => <button key={item.key} onClick={() => setTab(item.key)} className={`border-b-2 px-1 pb-3 text-sm font-medium transition ${tab === item.key ? "border-rosewood-500 text-rosewood-600" : "border-transparent text-stone-400 hover:text-stone-700"}`}>{item.label}</button>)}</div></div>
      ...
      {tab === "evolutions" && <section><div className="mb-5 flex items-center justify-between"><div><h2 className="font-semibold text-stone-800">Evoluções por sessão</h2><p className="text-sm text-stone-400">{patient.evolutions?.length || 0} registros clínicos</p></div><button className="btn-primary" onClick={() => openEvolution()}><Plus className="h-4 w-4" />Nova evolução</button></div>{patient.evolutions?.length ? <div className="space-y-4">{patient.evolutions.map((evolution) => <article key={evolution.id} className="card"><div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row"><div className="flex items-center gap-3"><div className="rounded-xl bg-rosewood-100 p-2.5 text-rosewood-500"><CalendarDays className="h-5 w-5" /></div><div><p className="font-semibold text-stone-700">{formatDate(evolution.sessionDate)} às {evolution.time}</p><p className="text-xs text-stone-400">{careLabels[evolution.careType]}</p></div></div><div className="flex gap-1"><button className="rounded-lg p-2 text-stone-400 hover:bg-rosewood-50 hover:text-rosewood-600" onClick={() => openEvolution(evolution)}><Pencil className="h-4 w-4" /></button><button className="rounded-lg p-2 text-stone-400 hover:bg-red-50 hover:text-red-500" onClick={() => removeEvolution(evolution.id)}><Trash2 className="h-4 w-4" /></button></div></div><div className="grid gap-4 md:grid-cols-2"><TextBlock title="Objetivo" text={evolution.sessionGoal} /><TextBlock title="Atividades realizadas" text={evolution.activitiesPerformed} /><TextBlock title="Desempenho" text={evolution.patientPerformance} /><TextBlock title="Progresso percebido" text={evolution.perceivedProgress} /><TextBlock title="Dificuldades observadas" text={evolution.observedDifficulties} /><TextBlock title="Próximos passos" text={evolution.nextSteps} /></div></article>)}</div> : <EmptyState title="Nenhuma evolução registrada" description="Registre o acompanhamento de uma sessão para construir a linha de cuidado." action={<button className="btn-primary" onClick={() => openEvolution()}><Plus className="h-4 w-4" />Registrar evolução</button>} />}</section>}
...
function Overview({ patient }: { patient: Patient }) {
  return <div className="grid gap-6 xl:grid-cols-3"><section className="card xl:col-span-2"><h2 className="mb-5 font-semibold text-stone-800">Informações clínicas</h2><div className="grid gap-5 md:grid-cols-2"><TextBlock title="Queixa inicial" text={patient.initialComplaint} /><TextBlock title="Diagnóstico ou condição principal" text={patient.mainCondition} /><TextBlock title="Histórico clínico" text={patient.clinicalHistory} /><TextBlock title="Observações gerais" text={patient.generalNotes} /></div></section><aside className="space-y-6"><div className="card"><h2 className="mb-4 font-semibold text-stone-800">Dados pessoais</h2><dl className="space-y-3 text-sm"><Detail label="Nascimento" value={formatDate(patient.birthDate)} /><Detail label="Responsável" value={patient.guardian} /><Detail label="Telefone" value={patient.phone} /><Detail label="E-mail" value={patient.email} /><Detail label="Endereço" value={patient.address} /></dl></div><div className="card bg-rosewood-600 text-white"><UserRoundCheck className="mb-3 h-6 w-6 text-rosewood-200" /><p className="text-2xl font-semibold">{patient.evolutions?.length || 0}</p><p className="text-sm text-rosewood-100">evoluções registradas</p></div></aside></div>;
}
